...
using Table = SunmiPOSLib.Models.Table;
...
public partial class TablePage : ContentPage
{
    ...
    private FlexLayout RowFlex()
    {
        FlexLayout flexRow = new FlexLayout()
        {
            ...
            Children =
                {
                    new Label() { Text = "Row." + rows.ToString(), FontSize = 16, TextColor = Colors.White, Padding=new Thickness(0, 4, 0, 0)},
                }
        };
    ...
    {
        var content1 = new Entry() { Text = "text", TextColor = Colors.White, WidthRequest = 60 };
        var content2 = new Entry() { Text = "text", TextColor = Colors.White, WidthRequest = 60 };
        var content3 = new Entry() { Text = "text", TextColor = Colors.White, WidthRequest = 60 };
        FlexLayout flexConteudo = new FlexLayout()
        {
            JustifyContent = FlexJustify.SpaceAround,
            Children =
                {
                    new Label() { Text="Content", TextColor=Colors.White, FontSize=18, Padding=new Thickness(10, 8, 0, 0) },
                    content1,
                    content2,
                    content3,
                }
    ...
    {
        var width1 = new Entry() { Text = "1", TextColor = Colors.White, WidthRequest = 60, Keyboard = Keyboard.Numeric, HorizontalTextAlignment = TextAlignment.Center };
        var width2 = new Entry() { Text = "1", TextColor = Colors.White, WidthRequest = 60, Keyboard = Keyboard.Numeric, HorizontalTextAlignment = TextAlignment.Center };
        var width3 = new Entry() { Text = "1", TextColor = Colors.White, WidthRequest = 60, Keyboard = Keyboard.Numeric, HorizontalTextAlignment = TextAlignment.Center };
        FlexLayout flexWeight = new FlexLayout()
        {
            JustifyContent = FlexJustify.SpaceAround,
            Children =
                {
                    new Label() { Text="weight%", TextColor=Colors.White, FontSize=18, Padding=new Thickness(20, 8, 0, 0), HorizontalOptions=LayoutOptions.End },
                    width1,
                    width2,
                    width3
                }
    ...
    private StackLayout AlignStack()
    {
        StackLayout stackAlign = new StackLayout()
        {
            ...
            Children =
                {
                    new Label() { Text="Alignment", TextColor=Colors.White, FontSize=18 },
                }
        };

        Label alignLabel1 = new Label { Text = "Left", TextColor = Colors.Red, FontSize = 15, Padding = new Thickness(0, 4, 0, 0) };
        ...
        Label alignLabel2 = new Label { Text = "Left", TextColor = Colors.Red, FontSize = 15, Padding = new Thickness(0, 4, 0, 0) };
        ...
        Label alignLabel3 = new Label { Text = "Left", TextColor = Colors.Red, FontSize = 15, Padding = new Thickness(0, 4, 0, 0) };
        ...
        tapGestureRecognizer1.Tapped += async (s, e) =>
        {
            ...
            string option = await DisplayActionSheet("Alignment", "cancel", null, aligns);
    ...
    private List<Table> GetTablesEntity()
    {
        ...
        for (int i = 0; i < rows; i++)
        {
            ...
            var align1 = Alignments[i][0].Text == "Left" ? AlignmentEnum.LEFT :
                Alignments[i][0].Text == "Center" ? AlignmentEnum.CENTER : AlignmentEnum.RIGHT;
            var align2 = Alignments[i][1].Text == "Left" ? AlignmentEnum.LEFT :
                Alignments[i][1].Text == "Center" ? AlignmentEnum.CENTER : AlignmentEnum.RIGHT;
            var align3 = Alignments[i][2].Text == "Left" ? AlignmentEnum.LEFT :
                Alignments[i][2].Text == "Center" ? AlignmentEnum.CENTER : AlignmentEnum.RIGHT;
    ...
    private async void OnPrint(object sender, EventArgs e)
    {
        ...
        try
        {
            var wasSucessful = SunmiPrinter.Current.AdvancePaper();
        }
        catch (Exception exception)
        {
            await DisplayAlert("Error", exception.Message, "ok");
